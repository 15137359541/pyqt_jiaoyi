...
class AddressBook(QWidget):
    def __init__(self, parent=None):
        ...
        self.addButton.show()#show（）方法意味着addButton是可见的
        self.submitButton = QPushButton("&Submit")
        self.submitButton.hide()
        self.cancelButton = QPushButton("&Cancel")
        self.cancelButton.hide()
        self.nextButton = QPushButton("&Next")
        self.nextButton.setEnabled(False)#被禁用，因此用户无法与它们进行交互。
    ...
    def addContact(self):
        ...
        self.nameLine.setFocus(Qt.OtherFocusReason)#设置焦点，即我们的光标指向  nameLine的位置。
    ...
    def cancel(self):
        ...
        number = len(self.contacts)
        self.nextButton.setEnabled(number > 1)#此变量中至少有2个条目，则启用下一个和上一个按钮。
        self.previousButton.setEnabled(number > 1)
