        ...
        if flag:
            ...
            for i in range(len(self.res)):
                ...
                if dict2 not in self.balance:
                    ...
                    # print(res)
                    if dict2['action'] =='BUY':
                        ht().ht_buy(security=dict2['action'],price= dict2['price'],amount = dict2['amount'])
                    elif dict2['action'] =='SELL':
                        ht().ht_sell(security=dict2['action'], price=dict2['price'], amount=dict2['amount'])
                else:
...
#以华泰为接口为例，其他同花顺使用方式同理
class ht():
    def __init__(self):
        self.user = easytrader.use('ht_client')
        #华泰的登陆
        self.user.prepare(user='666629279969', password='062929', comm_password="062929AAA")

    def ht_buy(self,security,price,amount):
        self.user.buy(security=security,price=float(price),amount = float(amount))

    def ht_sell(self,security,price,amount):
        self.user.sell(security=security,price=float(price),amount = float(amount))
